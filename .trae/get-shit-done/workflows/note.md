...
- **Global scope**: `C:/projects/personal/lexcv/.trae/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/projects/personal/lexcv/.trae/notes/`)
...
2. Glob `C:/projects/personal/lexcv/.trae/notes/*.md` (if directory exists) — global notes
...
Global (C:/projects/personal/lexcv/.trae/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/projects/personal/lexcv/.trae/notes/` — works in any directory
