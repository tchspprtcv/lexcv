...
const ARTIFACT_DIR = "C:/Users/francisco.horta/.gemini/antigravity/brain/08b40211-0f2d-4da7-af2b-70a896376590";
const ARTIFACT_IMG_DIR = `${ARTIFACT_DIR}/images`;
const DOCS_DIR = "c:/Users/francisco.horta/Documents/projects/personal/lexcv/docs";

function getManualMarkdown(type = 'artifact') {
  const imgPath = (name) => {
    if (type === 'artifact') {
      // Correct Windows absolute path without leading slash
      return `${ARTIFACT_IMG_DIR}/${name}`;
    } else if (type === 'docs_relative') {
      return `images/${name}`;
    }
    return `images/${name}`;
  };

  return `# Manual do Utilizador — Plataforma Jurídica ALCv
...
![Ecrã de Login Inicial](${imgPath('01_login.png')})

### 2.2 Credenciais de Acesso
* **Utilizador (Email):** Endereço de correio eletrónico profissional registado na plataforma (Ex.: \`admin@alcv.cv\` ou \`carlos.tavares@lexcv.cv\`).
...
![Matriz de Regras de Acesso (RBAC)](${imgPath('20c_rbac_permissoes.png')})
...
![Dashboard Institucional - Visão Superior](${imgPath('02_dashboard.png')})
...
![Dashboard Institucional - Visão Completa com Atividade Recente](${imgPath('02b_dashboard_completo.png')})
...
![Listagem Geral de Clientes](${imgPath('03_clientes_lista.png')})
...
![Formulário de Registo de Novo Cliente](${imgPath('04_clientes_novo.png')})
...
![Ficha Oficial do Cliente Formatada para Dossiê](${imgPath('05b_cliente_ficha.png')})

![Detalhe do Cliente com Abas e Conta-Corrente](${imgPath('05_cliente_detalhe.png')})
...
![Ferramenta de Merge / Fusão de Clientes](${imgPath('06_clientes_merge.png')})
...
![Listagem Geral de Processos Judiciais](${imgPath('07_processos_lista.png')})
...
![Wizard de Novo Processo - Etapa 1: Intake](${imgPath('08_processos_novo.png')})
...
![Detalhe do Processo - Visão Geral e Conflict Check](${imgPath('09_processo_detalhe.png')})
...
![Separador Timeline](${imgPath('09_tab_timeline.png')})
...
![Separador de Partes Processuais](${imgPath('09_tab_partes.png')})
...
![Separador de Fases Processuais](${imgPath('09_tab_fases.png')})
...
![Separador de Decisões Judiciais](${imgPath('09_tab_decisoes.png')})
...
![Separador de Factos do Processo](${imgPath('09_tab_factos.png')})
...
![Separador de Testemunhas](${imgPath('09_tab_testemunhas.png')})
...
![Separador de Documentos do Processo](${imgPath('09_tab_documentos.png')})
...
![Separador de Auditoria e Conformidade](${imgPath('09_tab_auditoria.png')})
...
![Emissão do Termo de Honorários](${imgPath('09b_processo_termo_honorarios.png')})
...
![Dashboard Operacional de Processos](${imgPath('10_processos_dashboard.png')})
...
![Agenda Forense Institucional](${imgPath('11_agenda.png')})
...
![Formulário de Agendamento de Novo Evento / Prazo](${imgPath('12_agenda_novo.png')})
...
![Repositório Central de Documentos](${imgPath('13_documentos.png')})

![Formulário de Upload de Documento com Versionamento](${imgPath('14_documentos_novo.png')})
...
![Painel de Controlo Financeiro e Honorários](${imgPath('15_financeiro.png')})

![Lançamento de Novo Honorário Associado a Processo](${imgPath('16_financeiro_novo.png')})
...
![Listagem e Gestão de Pareceres Jurídicos](${imgPath('17_pareceres.png')})

![Registo de Nova Solicitação de Parecer Jurídico](${imgPath('18_pareceres_novo.png')})
...
![Central de Notificações e Alertas de Prazos](${imgPath('19_notificacoes.png')})
...
![Definições de Sistema - O Meu Perfil](${imgPath('20_configuracoes.png')})

![Gestão de Utilizadores e Credenciais do Escritório](${imgPath('20b_gestao_utilizadores.png')})
...
// 1. Write to docs/MANUAL_DO_UTILIZADOR.md
const docPath = path.join(DOCS_DIR, "MANUAL_DO_UTILIZADOR.md");
fs.writeFileSync(docPath, getManualMarkdown('docs_relative'), 'utf8');
console.log("Wrote:", docPath);

// 2. Write to brain artifact manual_do_utilizador.md (Windows absolute path, no leading slash)
const artifactPath = path.join(ARTIFACT_DIR, "manual_do_utilizador.md");
fs.writeFileSync(artifactPath, getManualMarkdown('artifact'), 'utf8');
console.log("Wrote:", artifactPath);

// 3. Generate self-contained HTML version with embedded Base64 images
console.log("Generating standalone HTML with Base64 embedded images...");
...
<head>
  ...
  <style>
    ...
    .prose img { border-radius: 0.5rem; border: 1px solid #1e293b; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.3); margin-top: 1rem; margin-bottom: 1.5rem; max-width: 100%; height: auto; }
    .alert-box { border-left: 4px solid; padding: 1rem; border-radius: 0.375rem; margin: 1rem 0; }
    .alert-important { border-color: #3b82f6; background-color: rgba(59, 130, 246, 0.1); }
    .alert-note { border-color: #10b981; background-color: rgba(16, 185, 129, 0.1); }
  ...
  <div class="max-w-4xl mx-auto prose prose-invert">
    <header class="border-b border-slate-800 pb-6 mb-8">
      <div class="flex items-center gap-3 mb-2">
        <span class="bg-blue-600 text-xs font-semibold px-2.5 py-1 rounded text-white tracking-wide uppercase">Oficial</span>
        <span class="text-slate-400 text-sm">Versão 1.0 • Cabo Verde</span>
      </div>
      <h1 class="text-3xl md:text-4xl font-extrabold text-white tracking-tight">Manual do Utilizador — Plataforma Jurídica ALCv</h1>
      <p class="text-slate-400 mt-2">Guia ilustrado com capturas reais de ecrã recolhidas diretamente de <a href="https://www.alcv.tech" target="_blank" class="text-blue-400 hover:underline">www.alcv.tech</a>.</p>
    </header>
`;

function getBase64Img(imgName) {
  const p = path.join("c:/Users/francisco.horta/Documents/projects/personal/lexcv/docs/images", imgName);
  if (fs.existsSync(p)) {
...
const mdLines = getManualMarkdown('docs_relative').split('\n');
for (let line of mdLines) {
  if (line.startsWith('# ')) {
    continue; // already in header
  } else if (line.startsWith('## ')) {
    htmlContent += `<h2 class="text-2xl font-bold text-white mt-10 mb-4 border-b border-slate-800 pb-2">${line.replace('## ', '')}</h2>\n`;
  } else if (line.startsWith('### ')) {
    htmlContent += `<h3 class="text-xl font-semibold text-blue-300 mt-6 mb-3">${line.replace('### ', '')}</h3>\n`;
  } else if (line.startsWith('#### ')) {
    htmlContent += `<h4 class="text-lg font-medium text-slate-200 mt-4 mb-2">${line.replace('#### ', '')}</h4>\n`;
  } else if (line.startsWith('![') && line.includes('](')) {
    const caption = line.substring(2, line.indexOf(']('));
    const imgFile = line.substring(line.indexOf('](') + 2, line.length - 1).replace('images/', '');
    const b64 = getBase64Img(imgFile);
    if (b64) {
      htmlContent += `<div class="my-6"><img src="${b64}" alt="${caption}" class="w-full rounded-lg border border-slate-800 shadow-2xl" /><p class="text-xs text-center text-slate-400 italic mt-1.5">${caption}</p></div>\n`;
    }
  } else if (line.startsWith('> [!IMPORTANT]')) {
    htmlContent += `<div class="alert-box alert-important"><strong class="text-blue-400 block mb-1">IMPORTANTE</strong>`;
  } else if (line.startsWith('> [!NOTE]')) {
    htmlContent += `<div class="alert-box alert-note"><strong class="text-emerald-400 block mb-1">NOTA</strong>`;
  } else if (line.startsWith('> [!WARNING]')) {
    htmlContent += `<div class="alert-box alert-warning"><strong class="text-amber-400 block mb-1">ATENÇÃO</strong>`;
  } else if (line.startsWith('> ')) {
    htmlContent += `<p class="text-sm text-slate-300 my-1">${line.replace('> ', '')}</p>\n`;
  } else if (line.trim() === '---') {
    htmlContent += `<hr class="border-slate-800 my-8" />\n`;
  } else if (line.startsWith('|')) {
    // skip raw markdown table or render basic
    htmlContent += `<div class="text-xs text-slate-400 font-mono overflow-x-auto my-2">${line}</div>\n`;
  } else if (line.trim().length > 0) {
    htmlContent += `<p class="text-slate-300 leading-relaxed mb-3">${line}</p>\n`;
  }
}

htmlContent += `
  <footer class="mt-16 pt-8 border-t border-slate-800 text-center text-xs text-slate-500">
    © 2026 ALCv — Todos os direitos reservados. Plataforma de Gestão Forense para Cabo Verde.
  </footer>
  </div>
</body>
</html>`;

fs.writeFileSync(path.join(DOCS_DIR, "MANUAL_DO_UTILIZADOR.html"), htmlContent, 'utf8');
fs.writeFileSync(path.join(ARTIFACT_DIR, "manual_do_utilizador.html"), htmlContent, 'utf8');
