...
- [x] **LP-01**: Existe um novo endpoint público `GET /api/v1/public/branding` que devolve exclusivamente `nome` e `logoDataUrl` da tenant (nunca NIF, email, telefone ou qualquer outro campo), via uma DTO explícita — nunca a entidade `Tenant` diretamente
- [x] **LP-02**: O endpoint está registado na allowlist do `SecurityConfig` como uma entrada exata (nunca um wildcard tipo `/api/v1/public/**`)

### Webpage (Nova App Next.js Standalone)

- [x] **LP-03**: Existe uma nova aplicação Next.js standalone `webpage/` (independente da app `web/` existente), que serve a landing page pública
- [x] **LP-04**: Utilizador (autenticado ou não) que visita `/` vê sempre a landing page completa — nunca é redirecionado automaticamente para `/dashboard` ou `/login` só por ter sessão ativa
- [x] **LP-05**: Se o sistema ainda não foi inicializado (wizard `/setup` nunca correu), visitar `/` redireciona para `/setup`
- [x] **LP-06**: Quando o sistema está inicializado, a landing mostra o nome e o logo da tenant (via o endpoint da LP-01)
- [x] **LP-07**: A landing tem uma secção Hero com título, subtítulo e proposta de valor
- [x] **LP-08**: A landing tem uma secção de Funcionalidades/Módulos, destacando Clientes, Processos, Agenda/Prazos, Documentos, Financeiro e Notificações
- [x] **LP-09**: A landing tem uma secção de Prova Social/Confiança Institucional, com mensagens verificáveis (isolamento de dados multi-tenant, RBAC, auditoria, ecossistema NOSi/Cabo Verde) — nunca testemunhos ou logótipos de clientes fabricados
- [x] **LP-10**: A landing tem uma secção de Contacto/Pedir Demonstração com um link `mailto:` fixo (não sourced de `Tenant.email`/`telefone`)
- [x] **LP-11**: O CTA principal "Entrar" é um link `<a href="/login">` simples (nunca `next/link`/navegação client-side, por ser cross-zone), visível no topo e no fundo da página
- [x] **LP-12**: A landing é responsiva e suporta dark/light mode, reutilizando os componentes shadcn/ui e convenções Tailwind já usados em `web/`
...
- [x] **LP-14**: O Caddy roteia `/` (e o `assetPrefix` da webpage) para o novo container `webpage`; todas as outras rotas (`/login`, `/dashboard`, `/setup`, `/api/*`) continuam a ir para `web/`/`backend/` inalteradas — atualizado consistentemente nas 3 fontes de configuração existentes (`Caddyfile`, `Caddyfile.prod`, o heredoc inline em `docker-compose.hostinger.yml`)
- [x] **LP-15**: Existe um novo serviço `webpage` nos 3 ficheiros docker-compose (dev/prod/hostinger), com um Dockerfile próprio (build multi-stage, `output: standalone`, mesmo padrão de `web/Dockerfile`)
- [x] **LP-16**: O pipeline de CI/CD (`deploy.yml`) constrói e publica a imagem `webpage` como um 3º artefacto, ao lado de `backend`/`web`
...
|-------------|-------|--------|
| LP-01 | Phase 98 | Complete |
| LP-02 | Phase 98 | Complete |
| LP-03 | Phase 99 | Complete |
| LP-04 | Phase 99 | Complete |
| LP-05 | Phase 99 | Complete |
| LP-06 | Phase 99 | Complete |
| LP-07 | Phase 99 | Complete |
| LP-08 | Phase 99 | Complete |
| LP-09 | Phase 99 | Complete |
| LP-10 | Phase 99 | Complete |
| LP-11 | Phase 99 | Complete |
| LP-12 | Phase 99 | Complete |
| LP-13 | Phase 100 | Complete |
| LP-14 | Phase 100 | Complete |
| LP-15 | Phase 100 | Complete |
| LP-16 | Phase 100 | Complete |
