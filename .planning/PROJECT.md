...
**Current focus:** Milestone v2.10 (Notificações e Alertas) em execução — Phase 88/89 (Verificação Diária de Prazos e Honorários) concluída; primeiro job `@Scheduled`/cross-tenant deste código-base (`AlertasDiariosJob`, cron diário 06:00 `Atlantic/Cape_Verde`), com isolamento de falha em 4 camadas (job/tenant/categoria/entidade), idempotência por `categoria` (edge-triggered, com constraint DB de reforço) e sem qualquer dependência de `SecurityContextHolder`. **Primeira fase da milestone com verificação `passed` limpa, sem itens humanos pendentes** — puro backend, coberto integralmente pelos 44 testes automatizados. Próxima: Phase 89 (Sino e Página de Notificações) — última fase, a camada de consumo visual de tudo o que foi construído nas Phases 85-88.
...
---
*Last updated: 2026-07-09 — after Phase 88 (Verificação Diária de Prazos e Honorários)*
