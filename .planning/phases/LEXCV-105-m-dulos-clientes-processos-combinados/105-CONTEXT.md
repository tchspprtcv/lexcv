...
- Inconsistência do `h1` (font-semibold em Clientes vs font-bold em Processos): reconciliar para `font-semibold` (600) em ambas as fichas, não `font-bold` (700) — correção lateral explicitamente autorizada pelo utilizador, mas o `gsd-ui-checker` bloqueou a redação literal original ("font-bold") por introduzir um 3º peso de fonte além do limite de 2 pesos (400/600) já declarado no design system desde a Phase 103; `font-semibold` cumpre a mesma intenção (unificar o peso do h1 entre as duas fichas) sem violar o limite
