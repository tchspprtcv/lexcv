...
**Goal**: Existe um papel `PLATAFORMA_ADMIN`, distinto do `ADMIN` de cada escritório, associado a uma tenant reservada "ALCv", com uma capacidade de backend para criar tenants adicionais sem depender do wizard `/setup` — que se mantém singleton, reservado só ao arranque inicial da própria plataforma.
**Depends on**: Phase 118
**Requirements**: PROV-01, PROV-06
**Success Criteria** (what must be TRUE):

  1. Existe um papel `PLATAFORMA_ADMIN` seedado (`DatabaseSeeder`), distinto de `ADMIN`, e uma tenant reservada "ALCv" à qual os utilizadores desse papel pertencem
...
- [x] 119-01-PLAN.md — Seed do papel `PLATAFORMA_ADMIN` (zero permissões), da tenant reservada "ALCv" e do utilizador bootstrap, incondicional e idempotente, mais `TenantRepository.findByNome`
...
- [x] 120-02-PLAN.md — `PlatformAdminController`: `GET /platform/tenants` (com `countByTenantIdAndAtivoTrue` da Phase 117), `PUT /platform/tenants/{id}` (plano/limite) e `PATCH /platform/tenants/{id}/ativo` com guarda a impedir a suspensão da tenant reservada "ALCv"; DTOs próprios, handler global para corpo JSON inválido, e teste de gate por handler
...
**Goal**: Nenhuma superfície pública ou administrativa do produto continua a assumir que existe apenas um tenant — a landing pública mostra sempre a marca genérica ALCv, e a gestão de permissões por papel deixa de ser editável por cada escritório na interface, passando a ser uma operação fixa de plataforma. Esta fase corre imediatamente a seguir à Phase 120, nunca depois da Phase 122/123 — ver Risco abaixo.
**Depends on**: Phase 120
**Requirements**: ISOL-01, ISOL-02, ISOL-03
**Success Criteria** (what must be TRUE):

  1. `GET /api/v1/public/branding` deixa de depender de `TenantRepository.findFirstByOrderByCreatedAtAsc()` para decidir que marca mostrar — devolve sempre a marca genérica ALCv, independentemente de quantos tenants reais existirem
