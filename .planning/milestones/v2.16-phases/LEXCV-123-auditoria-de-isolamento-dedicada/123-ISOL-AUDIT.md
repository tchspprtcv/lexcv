...
| `PublicController.getBranding()` / `TenantPublicInfoResponse` | Resposta hardcoded (`nome("ALCv")`, `logoDataUrl(null)`), zero dependências injetadas, zero leitura de repositório ou `SecurityContextHolder` — nunca resolve nenhum tenant real (ISOL-01, fechado na Phase 121) | COVERED (marca genérica por design, não relacionado com PLATAFORMA_ADMIN) |
