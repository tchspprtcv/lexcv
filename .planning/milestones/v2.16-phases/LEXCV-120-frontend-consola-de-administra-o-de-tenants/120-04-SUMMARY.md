...
provides:
  - "web/src/app/(dashboard)/plataforma/columns.tsx: columns({ onEdit, onToggleAtivo }) factory producing 5 ColumnDef<TenantAdminSummary> (nome/plano/utilizadores/estado/acoes), plus the exported TENANT_RESERVADO = \"ALCv\" literal"
...
- The reserved "ALCv" tenant's Suspender action renders `disabled` and wrapped in the Phase 118 `<span tabIndex={0}>` composition inside `TooltipTrigger asChild`, with `TooltipContent` reading the byte-identical guard message `"Não é possível suspender o tenant da plataforma (ALCv)."` — proven to fire despite `buttonVariants`' `disabled:pointer-events-none`, since a bare `TooltipTrigger` on a disabled `Button` never would
