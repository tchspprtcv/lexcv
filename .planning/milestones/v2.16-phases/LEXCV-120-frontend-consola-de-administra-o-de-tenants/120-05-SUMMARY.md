...
- `web/src/app/(dashboard)/plataforma/page.tsx` created (25th route in `pnpm build`'s output, up from Plan 04's 24-route baseline): page-level RBAC mirror (`me.isFetched && !me.data?.roles?.includes("PLATAFORMA_ADMIN")` → `AccessDeniedState`), a `Tenants Registados` Card with client-side name search feeding both the shared `DataTable` (desktop, `hidden md:block`) and stacked cards (mobile, `md:hidden`, including its own `tabIndex={0}`-wrapped disabled-Suspender guard for the reserved "ALCv" row -- not just the desktop columns cell), and the `CriarTenantPanel` inline-toggle wired to `useCreateTenant` with cache invalidation and a success toast, zero `window.location.reload`
