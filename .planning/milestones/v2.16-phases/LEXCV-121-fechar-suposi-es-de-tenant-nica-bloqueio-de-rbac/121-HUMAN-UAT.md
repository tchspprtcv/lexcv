...
5. **CONFIRMADO (re-verificado após o fix WR-01 de `121-REVIEW.md`)** — A matriz continua viva: a tabela renderiza com as colunas de perfil. **Registo original (antes do fix WR-01):** clicado o checkbox de `TECNICO` na linha "Gerir Clientes" (estado inicial `false`/`disabled=false`) alternou para `true` — interação local sem persistência, aceite na altura como limitação conhecida (não havia botão de gravar). A revisão de código (`121-REVIEW.md`, WR-01) classificou isto como um Warning a corrigir — uma matriz "só de leitura" que continua a aceitar cliques é uma inconsistência de UX, mesmo sem risco de dados. **Corrigido e re-verificado ao vivo nesta sessão, após reiniciar o frontend (cache Turbopack corrompido, não relacionado com o fix — limpo com sucesso):** todos os 4 checkboxes da linha "Gerir Clientes" têm agora `disabled=true` (antes só o de `ADMIN` tinha); o checkbox de `TECNICO` mostra corretamente `checked=false` (não forçado a `true` só por estar desativado — a distinção `isAdminRow` vs `isDisabled` do fix está a funcionar); um clique programático confirmado a não alterar `checked` (`before === after`). O checkbox de `ADMIN` mantém-se `disabled=true`/`checked=true`, inalterado. A matriz é agora genuinamente só-de-leitura para um `ADMIN` de escritório, não apenas "sem botão de gravar."
