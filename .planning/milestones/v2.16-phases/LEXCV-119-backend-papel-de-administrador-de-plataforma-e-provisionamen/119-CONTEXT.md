...
- A reserved `Tenant` named literally `"ALCv"` plus one bootstrap `PLATAFORMA_ADMIN` user are also seeded unconditionally in `DatabaseSeeder` (NOT gated by `seedEnabled` — that flag controls optional demo data, e.g. `Cliente`/`Processo` fixtures; the reserved platform tenant is core infrastructure, needed for the capability to be usable at all, same category as the RBAC rows themselves).
  - > **⚠ PARCIALMENTE SUPERSEDED (revisao de planeamento, 2026-07-29) — ver `119-01-PLAN.md`.**
    > O **papel** `PLATAFORMA_ADMIN` e a **tenant reservada** `"ALCv"` continuam a ser seedados
    ...
    > por `nome = "ALCv"`, `Collections.emptyList()` no papel) mantem-se vinculativo.
- Idempotency: look up the reserved tenant by `nome = "ALCv"` before creating (mirrors the find-or-create idempotency already used by `upsertRolePermissions`) — repeated app restarts must never create duplicate reserved tenants.
