...
- **Fix attempted:** Added `--trust-lockfile` to the `pnpm install` line — pnpm's own documented flag for exactly this scenario ("Use only when the lockfile is part of the trusted base (closed-source projects, CI runs against an already-verified lockfile)" — ALCv is private/closed-source and this lockfile was frozen+audited in Phase 99). `--frozen-lockfile` itself still enforces the lockfile exactly matches `package.json`, so this flag would not have allowed any new or different package to be resolved.
