...
Plans:
- [x] 116-01-PLAN.md — Substituir a copy institucional (landing `trust-section.tsx` + `.trae/documents/SPEC.md`) pela formulação canónica do SIJ, generalizar a identidade do tenant de demonstração seedado (`DatabaseSeeder.java`: nome/NIF/tipo/email/telefone), e fechar formalmente SIJ-01 com asserção delimitada à secção "What This Is" + varrimento residual de "nosi" em todas as superfícies vivas
