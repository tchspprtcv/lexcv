...
- ✅ **v2.16 Distribuição Multi-Tenant e Faturação por Utilizadores** — Phases 117–124 (complete 2026-07-30)
...
<details>
<summary>✅ v2.16 Distribuição Multi-Tenant e Faturação por Utilizadores (Phases 117–124) — SHIPPED 2026-07-30</summary>

Evolução de "1 deployment por escritório" para uma instância partilhada multi-tenant, reabrindo deliberadamente a decisão da v2.12 de não ter provisionamento multi-tenant (base: `proposta_multitenancy_distribuicao_faturacao.md`). Sequência de dependência real: PLAN (117-118, limite de utilizadores + indicador) → PROV (119-120, papel `PLATAFORMA_ADMIN` + tenant reservada "ALCv" + consola de provisionamento) → Phase 121 (fecho de suposições de tenant única + bloqueio de `PUT /admin/rbac`, a correção de maior risco da proposta, corrida imediatamente a seguir à 120 por causa da janela de interferência entre tenants que só passa a existir a partir daí) → UTIL (122, relatório de utilização) → auditoria de isolamento dedicada (123, ISOL-04, no espírito da AUD-01 da v2.11 — confirma as 3 novas superfícies sem fuga entre tenants). A auditoria de integração cross-phase do fecho do marco encontrou uma duplicação de lógica não-bloqueante (o indicador "X/Y utilizadores" da Phase 118 recalculava a contagem no cliente em vez de reutilizar a fonte única do backend) — o utilizador escolheu inserir a Phase 124 para a fechar antes de encerrar o marco, em vez de a aceitar como dívida rastreada; confirmada ao vivo no browser sem regressão. 15/15 requisitos satisfeitos, 8/8 fases, 27/27 planos. Pré-condição de deployment antes de um 2º tenant pagante real: 3 migrações SQL manuais por correr em produção (ver `123-ISOL-AUDIT.md`). Ver Scope Note no archive.

- [x] Phase 117: Backend — Limite de Utilizadores por Tenant (PLAN-01, PLAN-02, PLAN-04) — 2/2 plans, completed 2026-07-29
- [x] Phase 118: Frontend — Indicador de Utilizadores no Limite (PLAN-03) — 3/3 plans, completed 2026-07-29
- [x] Phase 119: Backend — Papel de Administrador de Plataforma e Provisionamento (PROV-01, PROV-06) — 4/4 plans, completed 2026-07-29
- [x] Phase 120: Frontend — Consola de Administração de Tenants (PROV-02 a PROV-05) — 6/6 plans, completed 2026-07-29
- [x] Phase 121: Fechar Suposições de Tenant Única + Bloqueio de RBAC (ISOL-01, ISOL-02, ISOL-03) — 4/4 plans, completed 2026-07-29
- [x] Phase 122: Relatório de Utilização por Tenant (UTIL-01) — 4/4 plans, completed 2026-07-30
- [x] Phase 123: Auditoria de Isolamento Dedicada (ISOL-04) — 2/2 plans, completed 2026-07-30
- [x] Phase 124: Eliminar Duplicação da Contagem de Utilizadores Ativos no Indicador de Limite (INSERTED pós-auditoria, sem REQ-ID formal) — 2/2 plans, completed 2026-07-30

See archive: [milestones/v2.16-ROADMAP.md](milestones/v2.16-ROADMAP.md) · [milestones/v2.16-REQUIREMENTS.md](milestones/v2.16-REQUIREMENTS.md) · [milestones/v2.16-MILESTONE-AUDIT.md](milestones/v2.16-MILESTONE-AUDIT.md)
...
| 124. Eliminar Duplicação da Contagem de Utilizadores Ativos no Indicador de Limite | v2.16 | 2/2 | Complete   | 2026-07-30 |
...
Plans:
- [x] 124-01-PLAN.md — Backend: expor `tenant_utilizadores_ativos` em `GET /auth/me` a partir de `countByTenantIdAndAtivoTrue` (fonte única), com 4 testes Mockito e veredito do threat model
- [x] 124-02-PLAN.md — Frontend: indicador "X/Y utilizadores" passa a consumir esse campo, remove o `filter()` client-side e reescreve a asserção do gate da Fase 118 para proibir o regresso do filtro
