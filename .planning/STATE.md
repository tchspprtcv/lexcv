...
- `backend/migrations/120b-backfill-tenant-plano.sql` — manual-execution script, must be run against the database before/alongside next deploy (no migration runner in this repo). Until it runs against a given environment, any tenant row still holding a legacy `plano = NULL` (confirmed present in this dev DB) will render an incorrect badge in the `/plataforma` console — cosmetic only, `Tenant.plano`'s `@Builder.Default` already prevents any *new* tenant from ever being created with `plano = NULL`.
