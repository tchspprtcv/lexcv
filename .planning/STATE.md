...
- Phase 85 open product decision (`97-UAT.md`, carried forward unanswered): should an ALTA-priority `Evento` with null `dataFim` count as urgent in `agendaUrgentesCount`/`prazosCriticosCount`? Current behavior excludes it by design (see `isEventoCritico()` docblock in `ResourceController.java`, grep to relocate). Not resolved by v2.11 — awaiting a user product decision.
