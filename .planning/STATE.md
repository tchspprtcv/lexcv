...
- **(v2.13, 2026-07-17) Three previously-flagged background bugs each have an independent fix session that completed successfully** (per direct transcript review), but none have been merged into this repo's `master` yet — each lives on its own branch/worktree, started by the user outside this GSD orchestration, and merging is the user's call, not auto-applied here:
  ...
  **ATUALIZACAO 2026-09-23 (remocao dos worktrees) — estas tres linhas deixaram de ser verdade e
  ficam aqui corrigidas em vez de apagadas, porque descrevem bugs REAIS que continuam abertos.**
  Ao remover os worktrees a pedido do utilizador, varri todos os branches locais e remotos: os
  branches `claude/jovial-lederberg-d017b2` e `claude/mystifying-leavitt-6657b5` **já não existem**, e
  os directorios dos worktrees correspondentes tambem nao. O de `claude/sad-fermat-78bc39` ainda
  existia mas sem `.git` e, inspeccionado, **nao continha a correccao aplicada** (o gate
  ...
  **As correccoes estao perdidas** (nem branch nem directorio). Os bugs, nao. Quem os for fechar
  parte do zero, com as causas-raiz documentadas abaixo, que continuam corretas e uteis.
...
- ~~(v2.16, roadmap creation, 2026-07-28) ISOL-03 sequencing risk, carried into the roadmap itself: `PUT /api/v1/admin/rbac` must be locked to `PLATAFORMA_ADMIN` before any real 2nd paying tenant is provisioned~~ — **CLOSED (2026-07-29, Phase 121):** `updateRbac` locked to `PLATAFORMA_ADMIN` via method-level `@PreAuthorize`, shipped to `master` the same day as Phase 120, so no window ever existed in this repository's history with the console live and RBAC still tenant-editable (confirmed by commit-timestamp analysis during the v2.16 milestone integration audit). Phase 123's dedicated isolation audit re-confirmed no bypass exists. The remaining go/no-go pre-condition for a real 2nd tenant is now purely operational — see the 3 pending manual migration scripts above and `.planning/milestones/v2.16-MILESTONE-AUDIT.md`.
...
| Category | Item | Status |
|----------|------|--------|
| verification_gap | Phase 103 (103-VERIFICATION.md) — `human_needed`, 5/5 must-haves verified | non-blocking — residual live-checkpoint substitution after the Browser-pane instability later root-caused (hydration mismatch) this session. **Correccao 2026-09-23:** o branch `claude/sad-fermat-78bc39` que esta linha dava como contendo o fix ja nao existe, e o directorio do worktree, inspeccionado antes de ser removido, nao continha o fix aplicado. Tratar como nao corrigido. |
...
### Items acknowledged and deferred at milestone v2.16 close (2026-07-30)

Pre-close `audit-open` scan found 5 open items, all `uat_gap`, none BLOCKER-level (see `.planning/milestones/v2.16-MILESTONE-AUDIT.md` for full detail). Acknowledged and deferred rather than blocking close — all 5 are tool-detection artifacts, not genuine gaps:

| Category | Item | Status |
|----------|------|--------|
| uat_gap (tool false positive) | 118-HUMAN-UAT.md | Read in full this session — all points `CONFIRMADO`, 0 `FALHOU`. `audit-open` reports "status unknown, 0 pending scenarios" because this file has no machine-readable frontmatter status field the scan recognizes, not because a scenario is genuinely open |
| uat_gap (tool false positive) | 120-HUMAN-UAT.md | Same — 10 points `CONFIRMADO`, 0 `FALHOU`, read in full this session |
| uat_gap (tool false positive) | 121-HUMAN-UAT.md | Same — 8 points `CONFIRMADO`, 0 `FALHOU`, read in full this session |
| uat_gap (already resolved) | 122-HUMAN-UAT.md | A1-A3 `CONFIRMADO`; H1-H6 originally "NÃO VERIFICADO" (Browser MCP tooling blocker) — already formally resolved via 2 dated `overrides:` entries in `122-VERIFICATION.md`, explicitly accepted by the user on 2026-07-30. Not a new gap |
| uat_gap (tool false positive) | 124-HUMAN-UAT.md | 4 points `CONFIRMADO` (3 indicator states + reactivity), 0 `FALHOU` — live visual confirmation completed successfully this session |

Known deferred items count at v2.16 close: 5 (all uat_gap), 0 functional defects, 0 blocking. The milestone-level integration-checker additionally found 1 non-blocking cross-phase logic-duplication finding (Phase 118's indicator never reused Phase 117's single count source) — closed inline by the inserted Phase 124 rather than deferred, per the user's explicit choice; see `.planning/milestones/v2.16-MILESTONE-AUDIT.md`.
