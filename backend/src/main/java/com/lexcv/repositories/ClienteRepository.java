...
public interface ClienteRepository extends JpaRepository<Cliente, UUID> {
    ...
    // CAST-guarded, never skippable (PITFALLS.md Pitfall 1). termo is bound via
    // @Param and wildcard-wrapped INSIDE the SQL string as
    // '%' || CAST(:termo AS text) || '%' — never Java string-concatenated
    ...
    @Query(value = "SELECT c.* FROM t_cliente c " +
            ...
            "AND (unaccent(c.nome) ILIKE unaccent('%' || CAST(:termo AS text) || '%') " +
            "OR c.numero_cliente ILIKE '%' || CAST(:termo AS text) || '%' " +
            "OR c.nif ILIKE '%' || CAST(:termo AS text) || '%' " +
            "OR c.documento_numero ILIKE '%' || CAST(:termo AS text) || '%') " +
            "ORDER BY " +
            ...
            "WHEN c.numero_cliente ILIKE CAST(:termo AS text) || '%' THEN 1 " +
            "WHEN c.nif ILIKE CAST(:termo AS text) || '%' THEN 1 " +
            "WHEN c.documento_numero ILIKE CAST(:termo AS text) || '%' THEN 1 " +
            "WHEN unaccent(c.nome) ILIKE unaccent(CAST(:termo AS text) || '%') THEN 2 " +
            "ELSE 3 END, " +
            "c.created_at DESC " +
            "LIMIT :limit",
            nativeQuery = true)
    List<Cliente> pesquisarGlobal(@Param("tenantId") UUID tenantId, @Param("termo") String termo, @Param("limit") int limit);
}
