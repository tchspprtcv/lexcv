...
public interface ProcessoRepository extends JpaRepository<Processo, UUID> {
    ...
    @Query(value = "SELECT p.* FROM t_processo p " +
            ...
            "AND (p.numero_processo ILIKE '%' || CAST(:termo AS text) || '%' " +
            "OR unaccent(p.descricao) ILIKE unaccent('%' || CAST(:termo AS text) || '%') " +
            "OR unaccent(p.tribunal) ILIKE unaccent('%' || CAST(:termo AS text) || '%') " +
            "OR unaccent(p.area_juridica) ILIKE unaccent('%' || CAST(:termo AS text) || '%') " +
            "OR unaccent(p.tipo_processo) ILIKE unaccent('%' || CAST(:termo AS text) || '%')) " +
            "ORDER BY " +
            "CASE WHEN lower(p.numero_processo) = lower(CAST(:termo AS text)) THEN 0 " +
            "WHEN p.numero_processo ILIKE CAST(:termo AS text) || '%' THEN 1 " +
            "ELSE 2 END, " +
            "p.created_at DESC " +
            "LIMIT :limit",
            nativeQuery = true)
    List<Processo> pesquisarGlobal(@Param("tenantId") UUID tenantId, @Param("termo") String termo, @Param("limit") int limit);
}
